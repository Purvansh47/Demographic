...
# Demographic Table
  ...
  Tableby_grp = function(trt, var, data, name){
    ...
    colnames(Table1) = c("Statistics", "Group1", "Group2")
    Parameter = name
    Table1 = cbind(Parameter,Table1)
    Table1$Parameter = ifelse(Table1$Statistics %in% c("-  Female"), Parameter,
                            ifelse(Table1$Statistics %in% c("-  n"), Parameter,
                                   ifelse(Table1$Statistics %in% c("-  1"), Parameter,"")))
    Table1$Statistics = gsub("- ", "", Table1$Statistics)
    Table1$Group1 = gsub(" - ", ", ", Table1$Group1)
    Table1$Group2 = gsub(" - ", ", ", Table1$Group2)
    colnames(Table1) = c("Parameter", "Statistics", "Group 1", "Group 2")
    Table1 = Table1[-1, ]
    Table1
  ...
  Analysis = reactive({
    ...
    D = if (input$trt == "Without Treatment Group") {
      ...
      Data1 = Tableby_grp(as.character(Data$Treatment), as.character(Data$Sex), Data, "Gender n(%)")
      Data2 = Tableby_grp(as.character(Data$Treatment), as.numeric(Data$Age), Data, "Age (Years)")
      Data3 = Tableby_grp(as.character(Data$Treatment), as.numeric(Data$Height), Data, "Height (cm)")
      Data4 = Tableby_grp(as.character(Data$Treatment), as.numeric(Data$Weight), Data, "Weight (kg)")
      Data5 = data.frame(rbind(Data1, Data2, Data3, Data4))
      Data5
      Data5 = flextable(as.data.frame(Data5), cwidth = 2) %>%
                set_header_labels(Statistics = "Statistics",
